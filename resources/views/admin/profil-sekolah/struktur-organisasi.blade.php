@extends('layouts.app', ['title' => 'Kelola Struktur Organisasi - '])
@push('css')
    ...
    <header class="py-4">
        <div class="d-flex  justify-content-center">
            <div class="col-md-9">
                <div class="card">
                    <div class="card-body">
                        <h3>Kelola Halaman Struktur Organisasi</h3>
                        @if (session()->has('success'))
                            <div class="alert alert-success" role="alert">
                                {{session('success')}}
                            </div>
                        @endif
                        <form method="post" action="{{ route('struktur-organisasi.update',1) }}" enctype="multipart/form-data">
                            @method('PUT')
                            @csrf
                            <input type="hidden" name="page" value="struktur-organisasi">
                            <div class="form-group mb-3">
                                <label for="img" class="form-label">Bagan</label>
