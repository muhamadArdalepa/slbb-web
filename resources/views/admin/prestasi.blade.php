                        ...
                        <table id="Table" class="table">
                            ...
                            <tbody>
                                @foreach ($data as $i => $d)
                                    <tr>
                                        <td>{{ $i + 1 }}</td>
                                        <td style="white-space: nowrap">{{ $d->getRank() }}</td>
                                        <td>{{ $d->name }}</td>
                                        <td style="white-space: nowrap">{{ $d->type }}</td>
                                        <td style="white-space: nowrap">{{ $d->year }}</td>
                                        <td class="text-center" style="white-space: nowrap">
                                            <a href="{{ route('prestasi.edit', $d->id) }}"
                                                class="text-decoration-none p-1 text-primary">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                            <form class="d-inline" method="POST"
                                                action="{{ route('prestasi.destroy', $d->id) }}">
                                                @csrf
                                                @method('DELETE')
