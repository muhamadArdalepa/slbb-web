...
class PageController extends Controller
{
    ...
    function prestasi()
    {
        $data[0] = Prestasi::where('type', 1)->orderBy('year','desc')->get();
        $data[1] = Prestasi::where('type', 2)->orderBy('year','desc')->get();
    ...
    function kegiatanSiswa(Request $request)
    {
        ...
        if (auth()->user()->role == 3) {
            $kelas = Kelas::findOrFail(auth()->user()->kelas_id);
            $data = KegiatanSiswa::where('kelas_id', auth()->user()->kelas_id)->get();
